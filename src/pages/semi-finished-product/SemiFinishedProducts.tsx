...
const SemiFinishedProducts = () => {
  ...
  const { data, loading, setPage, totalPages, page, size } = usePaginatedData("/material/search?type=SEMI_FINISHED");
      ...
      customColumn: (row) => {
        return (
          <Flex gap="10px" align="center" justify="center">
            <Icon onClick={() => navigate(`/semi-finished-products/edit/${row.id}`)} icon="icon-pencil" />
  ...
  return (
    <>
      <div className="main-container">
        <PageTitle type="list" label="Yarm tayyor mahsulot" actions={<Button onClick={() => navigate("/semi-finished-products/add")} size="small" icon={<Icon icon="icon-plus" color={theme.buttonTextColor} />} />} />
      </div>
